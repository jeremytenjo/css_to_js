const isCSS = (item) => {
  const isCssRegx = item.match(/;/g) || [];
  return isCssRegx.length === 1;
};
...
const toJS = (item) => {
  ...
  return `${getBeginningWhitespace(prop)}${toCamel(prop.trim())}: "${val
    .trim()
    .replace(';', '')}",`;
};
...
const toCSS = (item) => {
  const [prop, val] = item.split(':');
  return `${getBeginningWhitespace(prop)}${toHyphen(prop.trim())}: ${val
    .trim()
    .replace(/'|"|,/g, '')};`;
};
